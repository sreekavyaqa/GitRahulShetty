...
//Count the number of names starting with Alphabet A in the list
	...
	public void streamCollect() {
		//list to new list -do manipulations-
		List<String>ls=Stream.of("Radhe","Krishna","BalaRam","Keshav","Rama","sita").filter(s->s.endsWith("a")).map(s->s.toUpperCase())
		.collect(Collectors.toList());
		System.out.println(ls.get(0));
		
		List<Integer>values=Arrays.asList(3,2,2,7,5,5,1,9,7);
		//print unique numbers and sort the array
		//values.stream().distinct().forEach(s->System.out.println(s));
		List<Integer> li=values.stream().distinct().sorted().collect(Collectors.toList());
		System.out.println("After sorting get 3rd index "+li.get(2));
	}
}
